...
Many of the common challenges have already been solved and extracted to reusable functions.
...
It's easiest to use the `withTelemetry` wrapper function that's available in the `@sentry/wizard` package:
...
If you use the `withTelemetry` helper, it will automatically filter out stack traces for you.
