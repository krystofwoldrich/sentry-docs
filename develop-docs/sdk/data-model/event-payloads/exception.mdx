...
Consequently, different SDKs took different approaches as to how they set the `type` attribute. 
In some cases, the `type` attribute was/is not set at all. 
Choosing [Trace Origin](/sdk/performance/trace-origin/) as the naming scheme for the `type` attribute means that we're using an already established and accepted naming scheme.
The scheme works well enough for the exception mechanism `type` attribute. 
Slight deviations to accommodate for the applicability to exceptions are allowed and to be expected.
